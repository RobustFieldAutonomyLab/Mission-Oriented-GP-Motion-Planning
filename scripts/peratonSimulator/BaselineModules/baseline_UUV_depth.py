...
class uuv:
    ...
    def __init__(self, z_control):
        ...
        wps = np.array([-0.0440566060897208,-0.0248240698620003,-0.107037692767652,-0.0219444642741428,-0.026825331751674,
        -0.0593671277427065,-0.02420744721168,-0.139899000380344,-0.05121187599767,-0.0205029830056119,-0.00603013441440597,
        -0.0251546268034846,-0.065016295425312,-0.032817561895604,-0.201382652390223,-0.01464347671504,-0.114590494075103])

        delta = np.array([0.056376715, 0.062458538,	0.004275523,0.037468322,0.015695149,
        -0.073516116, 0.005871452, 0.046592754,	0.049167087, 0.021494145,0.015998495,0.01695275,-0.051999475,0.070228969,
        	0.017897519,	-0.056531397,	-0.068477254])
            
        delta = np.array([1.91931391e-02,  2.81394271e-02, -1.49609075e-02, -7.42794112e-02,  9.09433151e-03,  2.90712709e-03,
                5.91784053e-03,  2.84221936e-02 , 2.86322722e-02,  5.85993683e-02, 1.32261491e-02 , 6.82011201e-02,  1.70863633e-02, -1.86479213e-02,
        ...
        self.scipy_threeDtrack = sci.RBFInterpolator(data_points, z_full, kernel='multiquadric',neighbors=1, epsilon =0, degree=0)
        self.tracker = []
        ...
        self.propEfficiency = np.array([0.77135637, 0.78763087, 0.791]) # found via OpenProp and our gear/motor matching algorithm
    ...
    def execute(self, stateVariables, tau, mission_data=None, sensor_data=None):
        ...
        if self.z_control:
            ...
            navZ = pts
            navZ = np.clip(navZ, ocean_depth + 0.005, -0.002)
            set_vel = self._set_speed * (np.array([navX-posX, navY-posY, navZ-posZ], np.dtype('float')) / np.sqrt((navX-posX)**2 + (navY-posY)**2 + (navZ-posZ)**2))
            ...
            gravity = 9.81 #m/s^2
            buoyant_accel = -1 *gravity * (1 - density * (volume/mass)) #1 kg/m^3 leads to ~ 1.3 km/hr in vel change
            ...
            def get_mass(var):
                ...
                buoyant_accel =  (60**4)/1000 * -1 *gravity * (1 - p_avg * (volume/avg_mass)) #standard SI units #NEGATIVE IS DOWN HERE
            ...
            ballastPower = abs(deltaMass/self.dt * gravity * Pressure)/100 #Power  in W is weight of liquid in unit time * Head in meters
            ...
            vec_angle = math.asin(float(np.dot(buoyant_speed, vehicle_speed))/float(np.linalg.norm(buoyant_speed)*np.linalg.norm(vehicle_speed)))
            ...
            set_vel = self._set_speed * (np.array([navX-posX, navY-posY]) / np.sqrt((navX-posX)**2 + (navY-posY)**2))
            ...
            vel_GP = self.planner.optimize(poses, vels, self.dt)
            ...
            if len(vel_GP) == 0:
                ...
                desired_vX = vel_GP[0][0]
                desired_vY = vel_GP[0][1]
            desired_vZ = 0
            accelX = self.deltaFromCurrentVal(vX, desired_vX)/self.dt
            accelY = self.deltaFromCurrentVal(vY, desired_vY)/self.dt
            accelZ = 0
        ...
        if self.z_control:
            self.log.append([tau, vehicle_accel, buoyant_accel, deltaMass, deltaMass/p_avg, mass/volume, self.drop_payload, self.AoA, pitch_angle])
    ...
    def terminate(self):
        ...
        data.to_csv('BuoyancyConsiderations.csv')
